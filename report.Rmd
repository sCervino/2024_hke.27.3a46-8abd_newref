...
model_settings <- list(
         ...
         recruitment_functional_form     = c('Mixed - BH, Rick, SegReg', "1978-2019", "This document", "Guidelines and year range that provides stable results"),
               parametric_uncertainty    = c('Boostrap', "1978-2019", "This document", "The aim of the mixed recruitment model is to add both structural and parametric uncertainty"),
                      ...
                      Natural_mortality  = c('historical mean', "2022", "ICES (2022)", "Low inter-annual variability"), 
                                Maturity = c('historical mean', "2021-2022", "ICES (2023)", "Low inter-annual variability"),
                          Weight_at_age  = c('historical mean', "2021-2022", "ICES (2023)", "Low inter-annual variability"),
                      Selection_pattern  = c('Boostrap of last 10 years F-at-age', "2013-2022", "ICES (2023)", "High interannual variability"),
...
# Model Settings
The settings of the simulations to calculate the reference points are the same used in the last benchmark of this stock (ICES, 2022) and are summarized in Table \@ref(tab:model-settings). The only difference is in the time period used for the calculation of stock-recruiment relationship which has been selected at the begining of this document based on the stability of the fits.


```{r model-settings, tab.cap='Simulation model settings', tab.id='model-settings', label='model-settings'}
scenario_table(model_settings, 
...
```
In this section we analysed if the SR breakpoint is well estimated. The parameters of the relationship were highly correlated `r corr_sr_pars`. This correlation made the fit sensitive to small variations in data. When one year is takend out of the fit (Figure \@ref(fig:breakpoint-1yrOut))  
...
<p class="refpt">
Based on long term stochastic simulations $F_{p.05} = $ `r Fp.05`. 
</p>

# Selection of Fmsy

The long term yields obtained for different values of fishing mortality using stochastic simulations are shown in Figure \@ref(fig:longtermyields). In curve of the median yield values around the maximum is very flat and a yield only 1% lower can be obtained in the whole range  [`r msy99[1]`,`r msy99[length(msy99)]`] (Figure \@ref(fig:)).
...
include_graphics("report/yield_equilibrium_bxp.png")
```

The theoretical $F_{msy}$ based on long term simulations is  $F_{msy} =$ `r Fmsy`. Far from the deterministic $F_{msy}$ values for each of the stock-recruitment relationships (\@ref(tab:fmsy-det)). The deterministic values in this case are much lower but are close to the lower bound of the fishing mortality range corresponding to yields no lower than $0.99\cdot MSY$.
